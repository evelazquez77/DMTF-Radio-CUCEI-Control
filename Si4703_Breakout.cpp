...
//Reads the strenght of current channel from STATUSRSSI
//Returns a number 
int Si4703_Breakout::getStrength() {
  readRegisters();
  int strength = si4703_registers[STATUSRSSI] & 0x00FF; //Mask out everything but the lower 8 bits
  return(strength);
}
    //Arreglar SBR para indicar STEREO
    //readRegisters();
    //if((si4703_registers[STATUSRSSI] & (1<<STEREO)) != 0) break;
